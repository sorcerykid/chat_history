--------------------------------------------------------
-- Minetest :: Chat History Mod v1.11 (chat_history)
--
...
local get_player_list = function( )
	local player_list = { }
	for i, p in ipairs( default.player_list ) do
		table.insert( player_list, { name = p.name, rank = p.rank } )
	end
	table.sort( player_list, function( a, b ) return a.name < b.name end )	-- sort modifies table, hence the need to create a local copy
	return player_list
end
...
--	formspec = formspec .. "textarea[0.3,0.3;11,7.5;buffer;Chat History (Last " .. ( #buffer == buffer_limit and buffer_limit or #buffer ) .. " Messages);\n"
	...
	for i, p in pairs( meta.player_list ) do
		formspec = formspec .. "," .. p.name .. ( { "", "", " \\[STAFF\\]", " \\[ADMIN\\]", " \\[OWNER\\]" } )[ p.rank ]
		if p.name == filter_player then
